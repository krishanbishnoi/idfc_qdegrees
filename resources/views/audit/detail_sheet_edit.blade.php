						...
						<div class="col-md-10 sp-row">
							...
							<div class="row flex-container mb-2">
								...
								<select class="form-control 0bervation" id="obs{{$value->id}}" data-id="{{$value->id}}" data-parameterId="{{$item->id}}" data-point="{{$value->weight}}">
										...
										@if($value->per==1)<option value="{{round(($value->weight))}}" data-type="rating" {{($resultSubPar[$value->id]->is_percentage==1)?'selected':''}}>Percentage</option>@endif
									</select>
									<span style="display:none" id="org{{$value->id}}">{{$value->weight}}</span>
								</div>
								<div class="col-md-2">
								<select class="form-control ratingSelect" name="ratingSelect" id="ratingSelect{{$value->id}}"  style="{{($resultSubPar[$value->id]->is_percentage!=1)?'display:none':'display:block'}}" data-id="{{$value->id}}" data-parameterId="{{$item->id}}">
									<option>select percentage</option>
									@for($counting=10;$counting<=100;$counting=$counting+10)
										<option value="{{$counting}}" {{($resultSubPar[$value->id]->selected_per==$counting)?'selected':''}}>{{$counting}}%</option>
									@endfor
								</select>
								@if($resultSubPar[$value->id]->is_percentage!=1)
								<input type="text" id="{{$value->id}}" readonly="readonly" class="form-control" value="{{ ($resultSubPar[$value->id]->is_critical==1)?'Critical':$resultSubPar[$value->id]->score}}"  style="{{($resultSubPar[$value->id]->is_percentage==1)?'display:none':'display:block'}}">
								@else
								<input type="text" id="{{$value->id}}" readonly="readonly" class="form-control" value="rating"  style="{{($resultSubPar[$value->id]->is_percentage==1)?'display:none':'display:block'}}">
								@endif
								</div>
								<div class="col-md-2">
