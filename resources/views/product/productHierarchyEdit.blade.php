                    ...
                    <div class=" form-group">
                        ...
                        <select name="branch" id="branch" data-placeholder="Choose a Branch..." class="standardSelect form-control" tabindex="3">
                            ...
                            @foreach($branch as $k=>$value)
                                <option value="{{$value->id}}" {{($bid==$value->id)?'selected':''}}>{{$value->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class=" form-group">
                        ...
                        <select name="type" id="type" data-placeholder="Choose a Product Type..." class="standardSelect form-control" tabindex="3">
                            ...
                            @foreach($product as $k=>$value)
                                <option value="{{$value->id}}"  {{($pid==$value->id)?'selected':''}}>{{$value->name}}({{($value->type==1)?'Recovery':'Regular'}})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                        ...
                        <select name="{{str_replace(' ','_',$item)}}" id="type" data-placeholder="Choose a {{$item}}..." class="standardSelect form-control" tabindex="3">
                            ...
                            @isset($finalUser[$item])
                                @foreach($finalUser[$item] as $k=>$value)
                                    <option value="{{$value->id}}" {{(isset($productUser[str_replace(' ',"_",$item)]) && $productUser[str_replace(' ',"_",$item)]==$value->id)?'selected':''}}>{{$value->name}}</option>
                                @endforeach
                            @endif
                        </select>
                        <input type="hidden" name="old[{{str_replace(' ','_',$item)}}]" value="{{isset($productUser[str_replace(' ',"_",$item)])?$productUser[str_replace(' ',"_",$item)]:''}}"/>
                    </div>
                </div>
