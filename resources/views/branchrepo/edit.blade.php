                    ...
                    <div class="col col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" id="text-input" name="name" placeholder="Branch repo Name"
                                                                class="form-control" value="{{$branchrepo->name}}">
                        </div>
                    </div>
                    <div class="col col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="branch_name" id="branch_name">
                                ...
                                @foreach ($branch as $item)
                                    <option value="{{$item->id}}" {{($branchrepo->branch_id==$item->id)?'selected':''}}>{{$item->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" id="text-input" name="location" placeholder="location"
                                                                class="form-control" value="{{$branchrepo->location}}">
                        </div>
                    </div>
...
@section('js')
    ...
    <script>
        jQuery(document).on('ready',function(e){
            getProduct("{{$branchrepo->branch_id}}");
        })
     ...
     function getProduct(id){
        jQuery.ajax({
                    ...
                    success: function (res) {
                        ...
                        jQuery('#product').val('{{$branchrepo->product_id}}')
                    }
