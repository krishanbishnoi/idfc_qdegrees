...
class AuditController extends Controller
{
    ...
    {
        //dd(all_non_scoring_obs_options(1));
        $result=Audit::with(['audit_parameter_result','audit_results'])->where('qm_sheet_id',Crypt::decrypt($qm_sheet_id))->first();
        ...
        $agency=Agency::whereIn('branch_id',$branch->pluck('id'))->get();
        $yard=Yard::whereIn('branch_id',$branch->pluck('id'))->get();
        // dd($result,$resultPar,$resultSubPar);
    	return view('audit.render_sheet_edit',compact('qm_sheet_id','data','result','resultPar','resultSubPar','branch','agency','yard'));
    }
    public function detail_audit_sheet_edit($qm_sheet_id)
    {
    	...
    	return view('audit.detail_sheet_edit',compact('qm_sheet_id','data','result','resultPar','resultSubPar','branch','agency','yard'));
    }
    ...
    public function audited_list()
    {
        ...
        if($ids->count()>0){
            $data = Audit::with(['qmsheet','product','branch.city.state','branch.branchable','yard.branch.city.state','agency.branch.city.state','qa_qtl_detail'])->whereNotIn('id',$ids->pluck('audit_id'))->get();
        }
        else{
            $data = Audit::with(['qmsheet','product','branch.city.state','branch.branchable','yard.branch.city.state','agency.branch.city.state','qa_qtl_detail'])->get();
        }
        // dd($data,$ids);
    ...
    public function done_audited_list()
    {
        $ids=Qc::with('user')->get()->keyBy('audit_id');
        if($ids->count()>0){
            $data = Audit::with(['qmsheet','product','branch.city.state','branch.branchable','yard.branch.city.state','agency.branch.city.state','qa_qtl_detail'])->whereIn('id',$ids->pluck('audit_id'))->get();
        }
        else{
            $data = Audit::with(['qmsheet','product','branch.city.state','branch.branchable','yard.branch.city.state','agency.branch.city.state','qa_qtl_detail'])->get();
        }
    ...
    public function audited_list_Post(Request $request)
    {
        ...
        $ids=Qc::all();
        if($ids->count()>0){
            $query = Audit::with(['qmsheet','product','branch','yard','agency'])->whereNotIn('id',$ids->pluck('audit_id'));
        }
        else{
            $query = Audit::with(['qmsheet','product','branch','yard','agency']);
        }
