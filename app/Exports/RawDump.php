...

{
    ...
    public function getGroupManager($branch_id, $product)
    {
...

    {
        ...
        if ($ids->count() > 0) {
            ...
            $data = Audit::with(['branchnew', 'audit_cycle', 'qmsheet' => function ($q) {
                $q->where('lob', $this->filterdata['lob']);
            }])->whereBetween('created_at', [$this->filterdata['start_date'], $this->filterdata['end_date']])->with(['audit_results.parameter_detail', 'audit_results.sub_parameter_detail', 'product', 'branch.city.state.region', 'branch.branchable', 'yard.branch.city.state.region', 'agency.branch.city.state.region', 'qa_qtl_detail', 'branchRepo.branch.city.state.region', 'agencyRepo.branch.city.state.region', 'user'])->withCount('artifact')->whereIn('id', $ids->pluck('audit_id'))->whereNotIn('id', $savedQcIds)->get(5);
            ...
            $data = Audit::with(['branchnew', 'audit_cycle', 'qmsheet' => function ($q) {
                $q->where('lob', $this->filterdata['lob']);
            }])->whereBetween('created_at', [$this->filterdata['start_date'], $this->filterdata['end_date']])->with(['audit_results.parameter_detail', 'audit_results.sub_parameter_detail', 'product', 'branch.city.state.region', 'branch.branchable', 'yard.branch.city.state.region', 'agency.branch.city.state.region', 'qa_qtl_detail', 'branchRepo.branch.city.state.region', 'agencyRepo.branch.city.state.region', 'user'])->withCount('artifact')->whereNotIn('id', $savedQcIds)->get(5);
        }
                ...
                if (!empty($row->qmsheet)) {
                    ...
                    switch ($row->qmsheet->type) {
                        ...
                        case 'agency':
                            ...
                            $name = $row->agency->name ?? 'N/A';

                            $code = $row->agency->agency_id ?? 'N/A';

                            $branch = $row->agency->branch->name ?? 'N/A';

                            $branchid = $row->agency->branch->id ?? 'N/A';

                            $state = $row->agency->branch->city->state->name ?? 'N/A';

                            $region = $row->agency->branch->city->state->region->name ?? 'N/A';

                            $report = $this->getReportingManager($branchid, $row->product_id);

                            $group = $this->getGroupManager($branchid, $row->product_id);
                        ...
                        case 'branch':
                            ...
                            $branchid = $row->branch->id ?? 'N/A';

                            $state = $row->branch->city->state->name ?? 'N/A';

                            $region = $row->branch->city->state->region->name ?? 'N/A';
                        ...
                        case 'repo_yard':
                            ...
                            $name = $row->yard->name ?? 'N/A';

                            $code = $row->yard->yard_id ?? 'N/A';

                            $branch = $row->yard->branch->name ?? 'N/A';

                            $branchid = $row->yard->branch->id ?? 'N/A';

                            $state = $row->yard->branch->city->state->name ?? 'N/A';

                            $region = $row->yard->branch->city->state->region->name ?? 'N/A';
                        ...
                        case 'branch_repo':
                            ...
                            $name = $row->branchRepo->name ?? 'N/A';
                            $code =  'N/A';

                            $branch = $row->branchRepo->branch->name ?? 'N/A';

                            $branchid = $row->branchRepo->branch->id ?? 'N/A';

                            $state = $row->branchRepo->branch->city->state->name ?? 'N/A';

                            $region = $row->branchRepo->branch->city->state->region->name ?? 'N/A';
                        ...
                        case 'agency_repo':
                            ...
                            $name = $row->agencyRepo->name ?? 'N/A';
                            $code =  'N/A';


                            $branch = $row->agencyRepo->branch->name ?? 'N/A';

                            $branchid = $row->agencyRepo->branch->id ?? 'N/A';

                            $state = $row->agencyRepo->branch->city->state->name ?? 'N/A';

                            $region = $row->agencyRepo->branch->city->state->region->name ?? 'N/A';
                    ...
                    if (!empty($this->filterdata['branch_name']) && ($branchid == $this->filterdata['branch_name'])) {
                        ...
                        foreach ($row->audit_results as $k => $value) {
                            ...
                            $final[] = [
                                ...
                                'qa' => $row->qa_qtl_detail->name ?? 'N/A',

                                'qc' => $ids[$row->id]->user->name  ?? 'N/A',

                                'cm' => $row->user->name ?? 'N/A',

                                'acm' => ($branchables->acm && $branchables->acm->name) ?  $branchables->acm->name : 'N/A',

                                'rcm' => ($branchables->rcm && $branchables->rcm->name) ? $branchables->rcm->name : 'N/A',

                                'zcm' => ($branchables->zcm && $branchables->zcm->name) ? $branchables->zcm->name : 'N/A',



                                'ncm' => ($branchables->ncm && $branchables->ncm->name) ? $branchables->ncm->name : 'N/A',

                                'group_head' => $group['name'] ?? 'N/A',
